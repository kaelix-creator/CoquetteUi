...
local Settings = lunarlib.Settings
local Flags = lunarlib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(lunarlib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then lunarlib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then lunarlib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then lunarlib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
	
	pcall(Save, "lunar library V5.json")
end
...
local Connections, Connection = {}, lunarlib.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(lunarlib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = lunarlib.Themes[lunarlib.Save.Theme]

local function AddEle(Name, Func)
	lunarlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = lunarlib.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function lunarlib:GetIcon(index)
...
function lunarlib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	lunarlib.Save.Theme = NewTheme
	SaveJson("lunar library V5.json", lunarlib.Save)
	Theme = lunarlib.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(lunarlib.Instances, function(_,Val)
...
function lunarlib:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function lunarlib:MakeWindow(Configs)
	local WTitle = Configs[1] or Configs.Name or Configs.Title or "lunar library V5"
	local WMiniText = Configs[2] or Configs.SubTitle or "by : kaelix999"
	...
	local UISizeX, UISizeY = unpack(lunarlib.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, lunarlib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			lunarlib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("lunar library V5.json", lunarlib.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		lunarlib.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("lunar library V5.json", lunarlib.Save)
	end)
	...
	function Window:CloseBtn()
		local Dialog = Window:Dialog({
			Title = "Lunar Client",
			Text = "Tem certeza que quer fechar o Lunar Client?",
			Options = {
				{"Sim", function()
					ScreenGui:Destroy()
				end},
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			lunarlib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(lunarlib.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(lunarlib.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(lunarlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(lunarlib.Options, {type = "Section", Name = SectionName, func = Section})
